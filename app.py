...
if uploaded_file := st.sidebar.file_uploader("Upload SalesData.csv", type=['csv']):
    ...
    with tab3:
        ...
        if not sk_filtered.empty:
            ...
            # 定义节点坐标: 左侧在 0.05, 右侧在 0.95，实现完美的左右对齐留白
            ...
            fig_sankey = go.Figure(data=[go.Sankey(
                ...
                node=dict(
                    pad=30, thickness=20,
                    label=[""] * len(nodes), # 核心：清除区块内文字，由标注替代
                    color=node_colors,
                    x=node_x, y=node_y,
            ...
            fig_sankey.update_layout(
                ...
                title_font=dict(size=18, color="#051C2C", family="Playfair Display"),
                plot_bgcolor='rgba(0,0,0,0)',
                paper_bgcolor='rgba(0,0,0,0)'
            )
            
            # 强制不使用 streamlit 主题，防止灰色文字背景注入
